...
void CONFIGS_USED::Read()
{
	...
	for (int i = 0; i < numSaveConfigs; ++i)
		lastConfigs.push_back(s.value(QString().setNum(i), "").toString());
	s.endGroup();
}
...
{
	int n;
	if ((n = CONFIGS_USED::lastConfigs.indexOf(parent->dsSrc.str)) < 0)
	{
		if (CONFIGS_USED::lastConfigs.size() == CONFIGS_USED::maxSavedConfigs)
			CONFIGS_USED::lastConfigs.removeLast();

		CONFIGS_USED::lastConfigs.insert(0, parent->dsSrc.str);
...
/*============================================================================
  ...
  * EXPECTS: path (may be null) if not null mast point to path ending in '/'
  *	RETURNS: nothing
  * REMARKS:
 *---------------------------------------------------------------------------*/
...
static void __ElemColorChanged(QSettings &s, _CElem &elem)
{
	...
	{	
		...
		if(elem.color.changed & 2)
			s.setValue("opacity", elem.color.opacity);	
		elem.color.changed = 0; 
		s.endGroup();	
	} 
	if (elem.background.changed)
	{
		s.beginGroup("background");
			...
			if (elem.background.changed & 2)
				s.setValue("opacity", elem.background.opacity);
			elem.background.changed = 0;
		s.endGroup();
	}
...
/*============================================================================
  ...
  * EXPECTS: nothing
  *	RETURNS: nothing
  * REMARKS: saves config in dsSrc directory!
...
{
	QString sName = dsSrc.str +  falconG_ini;
