...
LanguageTexts &LanguageTexts::operator=(const QStringList &txts)
{
	...
	{
		lenghts.push_back(txts[i].length());
...
void AlbumGenerator::_GetTextIDsFromStruct(FileReader &reader, IdsFromStruct & ids, int level)
{
	...
	do
	{
		...
		{												// it can be an ID or an image path relative to 'config.dsSrc'
			...
			// but image names in the source directory that starts with a decimal digit must be written after a space!
		...
		{
			if (reader.l()[level + 1] == DESCRIPTION_TAG[0])	// description line comes after the title lines
			...
			else if (reader.l()[level + 1] == TITLE_TAG[0])	// title lines come first: ids.what must be 'nothing'
			{
				ids.what = IdsFromStruct::title;
			}
				// common part for title and description
			int len = reader.l().length();
			while (len && reader.l()[--len] != ']')	// find last ']'
				;			
			++len;		// include closing ']' but not the possible ID/collision
			texts.SetTextForLanguageNoID(reader.l().mid(level + 10, len - level - 11), lang);
			if (config.majorStructVersion == 1)
			{				// none = there is no asterix followed by a number
				bool none = (textID==0) && (reader.l().length() == len);	
				if (config.minorStructVersion == 0)		// then can be an overflow number after a '*'
				{										// old version
					texts.collision = none ? 0 : reader.l().mid(len + 1).toInt(); // len+1: length including '*'
				}
				else if (config.minorStructVersion == 1)		
				{
					if (!textID)    // ID is saved at first language text only, so if 0 this is the first text line
					{
						...
						{
							textID = texts.ID = reader.l().mid(len + 1).toULongLong(); // len+1: length including '*'
...
*/
	...
	if (n != 9)		   // maybe new image added and not yet processed
	{
		...
		id = _AddImageFromPathInStruct(sl[0]);
		if (id)	
		{
			if (thumbnail)
				album.thumbnail = id;
...
bool AlbumGenerator::_ReadJalbum()
{
	...
	_structChanged = true;					// i.e. must write 'gallery.struct'
	return true;
}
...
int AlbumGenerator::_WriteGalleryContainer(const Album & album, bool itIsAnAlbum, int i)
{
	...
	{
		title = _textMap[pImage->titleID][_actLanguage];
		desc = _textMap[(pImage->descID)][_actLanguage];
...
/*============================================================================
* TASK:		read one image definition from the images.desc file
* EXPECTS:	reader - file open in 'reader'
*			img - output: image data
* GLOBALS:
* REMARKS:format of image definition lines:
*	<image name> (<image size>,<width>x<height>,<owidth>x<oheight>,<ipload date>) = > <original file name>
*--------------------------------------------------------------------------*/
void AlbumGenerator::_ImageFromImageDescFile(FileReader &reader, Image & img)
{
	int pos, pos1;

	pos = pos1 = reader.l().indexOf(' '); // after name	 (ID.extension)
	while (pos >= 0 && reader.l()[--pos] != '.')	 // get end of ID
		;
	img.ID = reader.l().mid(0, pos).toULongLong();		  // ID

	pos = pos1 + 2;	// after '('
	pos1 = reader.l().indexOf(',', pos);		// to file size
	img.fileSize = reader.l().mid(pos, pos1 - pos).toULongLong();
	pos = pos1 + 1;	// skip ','

	pos1 = reader.l().indexOf('x', pos);
	img.width = reader.l().mid(pos, pos1 - pos).toInt();  // width

	pos = ++pos1;    // to height
	pos1 = reader.l().indexOf(',', pos);
	img.height = reader.l().mid(pos, pos1 - pos).toInt(); // height

	pos = ++pos1;		// after the ',' to original width

	pos1 = reader.l().indexOf('x', pos);
	img.owidth = reader.l().mid(pos, pos1 - pos).toInt();  // original width

	pos = ++pos1;    // to original height
	pos1 = reader.l().indexOf(',', pos);
	img.oheight = reader.l().mid(pos, pos1 - pos).toInt(); // original height

	img.aspect = img.oheight ? (double)img.owidth / (double)img.oheight : 1.0;

	pos = ++pos1;	// to upload date
	pos1 = reader.l().indexOf(')', pos);
	img.uploadDate = DateFromString(reader.l().mid(pos, pos1 - pos));

	pos = pos1 + 5;		// skip ") => "						   // rel. path

	SeparateFileNamePath(reader.l().mid(pos), img.path, img.name);
	img.path = config.dsSrc.ToString() + img.path;
...
int AlbumGenerator::_ProcessImages()
{
	...
	for (auto im : _imageMap)
	{
		...
		if (dstExists)		// then test if this image was modified (width = 0: only added by name)
		{
			...
			if (dt <= im.uploadDate &&	fi.size() == im.fileSize)   // not newer and  same size  (TODO: checksum)
