...
//	connect(ui.tnvImages, &ThumbnailWidget::SignalTitleChanged,		this, &FalconG::_TrvTitleChanged);
	...
	if (CONFIGS_USED::lastConfigs.size() > CONFIGS_USED::indexOfLastUsed)
	...
	on_btnWeb_clicked();	// default selection to config parameters
}
...
/*============================================================================
* TASK:		Reads in  'images.desc' into '_imageMap'
* EXPECTS:
* GLOBALS:
* REMARKS:
*--------------------------------------------------------------------------*/
void FalconG::on_btnReadImg_clicked()
{
}
...
void FalconG::on_cbPointSize_currentTextChanged(const QString & txt)
{
	...
	pElem = _SelectSampleElement(pb);
	handler.Set(pb->styleSheet());
	handler.SetItem("QPushButton", "font-size", text);
...
void FalconG::on_edtFontFamily_textChanged()
{
	...
	handler.SetItem("QPushButton", "font-family", pElem->font.family);
	pb->setStyleSheet(handler.StyleSheet());
// DEBUG
...
*--------------------------------------------------------------------------*/
void FalconG::_SetCommonControlsFromWebButton(StyleHandler &shElem, QAbstractButton * pb, _CElem & elem)
{
	StyleHandler
...
* REMARKS:
*--------------------------------------------------------------------------*/
void FalconG::on_btnWeb_clicked()
{
...
void FalconG::on_btnShadowColor_clicked()
{
	...
	_SetChangedConfig();
	//handler.SetItem("QPushButton", "text-shadow", ")
}
...
void FalconG::_GetTextsForEditing(whoChangedTheText who)
{
	...
	if (who == wctBaseLangCombo)		// then just change base text for given language
	{
		...
		ui.edtBaseTitle->setPlainText(_selection.title[bli]);
		ui.edtBaseDescription->setPlainText(_selection.description[bli]);
		--_busy;
		return;
	}
	...
	{
		_SaveChangedTexts();					// actual text, which may be changed
		ui.edtTitleText->setPlainText(_selection.title[li]);
		ui.edtDescriptionText->setPlainText(_selection.description[li]);
		--_busy;
		return;
	}
...
void FalconG::on_edtTitleText_textChanged()
{
	...
	if (ui.cmbBaseLanguage->currentIndex() == ui.cmbLanguage->currentIndex())
		ui.edtBaseTitle->setDocument(ui.edtTitleText->document());
...
void FalconG::on_edtDescriptionText_textChanged()
{
	...
	if (ui.cmbBaseLanguage->currentIndex() == ui.cmbLanguage->currentIndex())
		ui.edtBaseDescription->setDocument(ui.edtDescriptionText->document());
...
* REMARKS:
*--------------------------------------------------------------------------*/
_CElem *FalconG::_SelectSampleElement(QAbstractButton *&pb)
{
...
void FalconG::_OpacityChanged(int val, int which)
{
	...
	handler.SetItem("QPushButton", (which == 1 ? "color" : "background-color:"), ss);
	pb->setStyleSheet(handler.StyleSheet());
...
void FalconG::_SetColorToConfig(StyleHandler &handler, _CElem &elem)
{
	...
	{	
		...
		elem.color.changed = true; 
		ui.btnSaveConfig->setEnabled(config.changed == true);  
		handler.SetItem("QPushButton", "color", ColorToStr(elem.color));
	}	
	...
	{	
		...
		elem.background.changed = true; 
		handler.SetItem("QPushButton", "background-color", ColorToStr(elem.background));
		ui.btnSaveConfig->setEnabled(config.changed == true);
	}
	if(elem.gradient.useEver && elem.gradient.changed)
	{
		...
		if (elem.gradient.used)
		{
			...
			handler.SetItem("QPushButton", "background", _GradientStyleQt(elem, false));
		...
		if (elem.border.changed)
		{
			...
			if (elem.border.used)
			{
				handler.SetItem("QPushButton", "border", "1px solid " + elem.border.color.name);
...
void FalconG::_SetupWebButtonFromConfig()
{
	...
	{
		handler.SetItem("QPushButton", "background", _GradientStyleQt(*from));				// unpressed
		handler.SetItem("QPushButton:pressed", "background", _GradientStyleQt(*from, true));	// pressed
		if (from->border.used)
		{
			handler.SetItem("QPushButton", "border", QString("1px solid %1").arg(from->border.color.name) );
	...
	if(!from->font.family.isEmpty() && from->font.family != handler.GetItem("QPushButton", "font-family"))
		handler.SetItem("QPushButton", "font-family", from->font.family);
	if (!from->font.sizestr.isEmpty() && from->font.sizestr != handler.GetItem("QPushButton", "font-size"))
		handler.SetItem("QPushButton", "font-size", from->font.sizestr);
	...
	else
	{
		...
		handler.SetItem("QPushButton", "text-decoration", "underline");
	}
}
