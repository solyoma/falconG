...
//	if (line.left(15) != "Language count:")
	...
	{
		for (int i = 0; i < langcnt; ++i)
			(*languages["falconG"])[i] = "Site generated by <strong>falconG</strong> - © A. Sólyom 2018-2022";
	}
...
int AlbumGenerator::_CreateAboutPages()
{
	...
	for (int i = 0; i < languages.LanguageCount(); ++i)
	{
		...
		if (_OutputAboutText(i) < 0)
			QMessageBox::warning(frmMain, tr("FalocnG - Warning"), QString(tr("Missing or unreadable \"about_text%1.txt\" file")).arg((*languages["abbrev"])[i]));
