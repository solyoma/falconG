...
*--------------------------------------------------------------------------*/
static IDVal_t CalcCrc(const QString& str, bool isContent)
{
	IDVal_t id;
	...
	return id;
		...
		// ******************* PathMap ********************
IDVal_t PathMap::_CalcId(const QString& path)
{
	IDVal_t id = CalcCrc(path, false);
...
QString PathMap::AbsPath(IDVal_t id) const
{
	QString path;
...
IDVal_t PathMap::Id(QString path) const
{
...
//}
//
//inline bool PathMap::Contains(ID_t id) const
//{
//	return _idToPath.contains(id);
//}

IDVal_t &PathMap::operator[](const QString &path)
{
	static IDVal_t dummy;
	if (path.isEmpty())
		return dummy;
...
QString &PathMap::operator[](IDVal_t id)
{   
...
QString PathMap::Insert(IDVal_t id, const QString& path)	// same id new path
{
 ...
 * REMARKS:
 *------------------------------------------------------------*/
void PathMap::Remove(IDVal_t id)
{
	if (_idToPath.contains(id))
	{
...
void ImageMap::Remove(IDVal_t id, bool isThumbnail)
{
	if (!contains(id))
		return;
...
/*=============================================================
 * TASK   : Set this album as an alias for another album
 *			and modify its aliasesList
 * PARAMS :	thatAlbum: a non zero pointer to the base album
 * EXPECTS:
 * GLOBALS:
 * RETURNS: none
 * REMARKS: - if this album is already an alias for thatAlbum
 *			  does nothing
 *			- if this alias to be has a list of items, i.e.
 *			  was a base album itself then 
 *				* those items are added to the items of the base album
 *				* and all albums that was an alias to this album 
 *					will be aliases of 'thatAlbum'
 *				* this will also be an alias to 'thatAlbum'
 *------------------------------------------------------------*/
void Album::SetAsAliasFor(Album* thatAlbum)		// thatAlbum must exist, pointer mustn't be 0
{
	if (!thatAlbum  || (thatAlbum && (baseAlbumId == thatAlbum->ID.Val()|| thatAlbum->ID.Val() == NO_ID)) )
		return;		// already an alias for that album or can't be an alias
	IDVal_t ourId = ID.Val();
	baseAlbumId = thatAlbum->ID.Val();			// new base album
	changed = true;	// this album changed
	if (items.count() > 0)	// if this album has items
	{
		thatAlbum->items += items; // add to 'thatAlbum' items and remove them from here
		items.clear();
	}
	if (aliasesList.count() > 0)	// if this album was a base album and had aliases
	{
		for (auto& idv : aliasesList)	// set the new base album for those albums
		{
			albumgen.Albums()[idv].baseAlbumId = baseAlbumId;
			albumgen.Albums()[idv].changed = true;
		}
		thatAlbum->aliasesList += aliasesList;	// and all of our aliases will be aliases to the new base album
	}
	thatAlbum->aliasesList += ourId;			// add this album to the aliases of 'thatAlbum' too
	thatAlbum->aliasesList.remove(baseAlbumId);	// remove 'thatAlbum' from its own aliases list
...
int Album::TitleCount()
{
	if (_titleCount < 0)
	{
		...
		for (auto &a : items)
			if ((a.Val()) &&
				(
					((a.IsAlbum()) && albumgen.Albums()[a.Val()].titleID) ||
					((a.IsVideo()) && albumgen.Videos()[a.Val()].titleID) ||
					((a.IsImage()) && albumgen.Images()[a.Val()].titleID)
				)
			)
				++_titleCount;
	}
...
int Album::DescCount()
{
	if (_descCount < 0)
	{
		...
		for (auto &a : items)
			if (a.Val() &&
				(
					((a.IsAlbum()) && albumgen.Albums()[a.Val()].descID) ||
					((a.IsVideo()) && albumgen.Videos()[a.Val()].descID) ||
					((a.IsImage()) && albumgen.Images()[a.Val()].descID)
				)
			)
				++_descCount;
	}
...
ID_t Album::ThumbID()
{
	...
	// from here no image or video is inside album, just sub-albums
	for (auto &iid : items)
...
ID_t Album::SetThumbnail(ID_t id)
{
	...
	albumgen.AddToModifiedList(ID.Val(), false);
	id.SetFlag(IDFlags(~TYPE_FLAGS), false);
	ID_t oldThumbnail = thumbnailId;
	thumbnailId = id;
	if(albumgen.Contains(id))
		++albumgen.ImageAt(id)->thumbnailCount;
	else
		albumgen.AddThumbnailBeforeImageRead(id.Val());
...
ID_t Album::SetThumbnail(IDVal_t id)
{
 ...
 *-------------------------------------------------------*/
ID_t Album::IdOfItemOfType(uint8_t type, int index)
{
	if (index >= items.size())
...
{
	QString s = HtmlNameFromID(ID.Val(), language, aliasRootId, http);
	if (http)
	{
...
* REMARKS:
*--------------------------------------------------------------------------*/
QString Album::HtmlNameFromID(IDVal_t id, int language, IDVal_t rootAliasId, bool withAlbumPath)
{
	QString s;
	...
	else
	{
		...
		else
		{
			...
			{
				QString s1 = QString("%1%2").arg(rootAliasId).arg(id);
				idt = GetUniqueAlbumID(albumgen.Albums(), s1, false); // get a unique ID for the alias
			}
			s = languages.FileNameForLanguage(QString("%1%2.html").arg(config.sBaseName.ToString()).arg(idt), language);
		}
...
* EXPECTS: 	parentID - ID of parent album, 0: topmost album
*			name: IN - either just a name, an absolute path name or a path relative to 
 ...
 * PARAMS:	id - id of album to remove. 
 * GLOBALS:
...
bool AlbumMap::RemoveRecursively(ID_t id)
{
	...
	if (id.Val())
	{
		...
		if ((album->ID.IsAlbum()) == 0)
			return false;
		Album* pBase = album->BaseAlbum();
		if (pBase)	 // if this album is an alias for another album
		{
			pBase->aliasesList.remove(id.Val());	// remove this album from the list of aliases of the base album
			remove(id.Val());						// and also remove this album from the map
			return true;					// no other processing needed
		}
		if (!album->aliasesList.isEmpty()) 	// then this album is a base album for other albums
		{
			Album* pNewBaseAlbum = Find(album->aliasesList[0]);	// get the first alias
			assert(pNewBaseAlbum);	// must be there
			album->SetAsAliasFor(pNewBaseAlbum);	// set this album as an alias for the new base album
													// so this album can be removed from the map
			pNewBaseAlbum->aliasesList.remove(id.Val());	// remove this album from the aliases of the new base album
			album->aliasesList.clear();				// and clear the aliases list of this album
			  // now this is a single album with no aliases
		}
			// if this album is not an alias for another album
											// then first remove its sub albums recursively
...
/*=============================================================
 * TASK   :	see if this album has a list of aliases and get their
 *			pointers into  a list
 * PARAMS :	id : id of album we search aleases for
 * EXPECTS:
 * GLOBALS:
 * RETURNS:
 * REMARKS:
 *------------------------------------------------------------*/
AlbumList AlbumMap::GetAliases(IDVal_t idv)
{
	if (idv == NO_ID || !contains(idv))
		return AlbumList();
	Album* p = &operator[](idv);

	AlbumList alst;

	if (p->baseAlbumId == NO_ID)	// this is the base album
		alst.SetBaseAlbumId(idv);	// set the base album
	else
		alst.SetBaseAlbumId(p->baseAlbumId);
...
* REMARKS:
*--------------------------------------------------------------------------*/
Album *AlbumMap::Find(IDVal_t id)
{
...
ID_t AlbumGenerator::ThumbnailID(Album& album, AlbumMap& albums)
{
	...
	{						  // will be set as its thumbnailId
		...
		if (!album.ImageCount() && !album.VideoCount()) // then get thumbnailId for first sub-album
			return ThumbnailID(albums[album.IdOfItemOfType(ALBUM_ID_FLAG, 0).Val()], albums);
		else
...
{
	++_structFileChangeCount;	 // always changed when a non-processed video line is in the structure
...
/*==========================================================================
* TASK:		add a sub-album or an image or video to global arrays and album 'parentID'
...
{
	Album* ab = AlbumForIDVal(parentID);
	Q_ASSERT(ab);
	...
	{
		if (type == ftImage || type == ftUnprocessedImage)	  // ftFolder is not possible here
...
bool AlbumGenerator::_IsAlbumAndItsSubAlbumsEmpty(Album& a)
{
	...
	{
		for (auto &b : a.items)
			if (!_IsAlbumAndItsSubAlbumsEmpty(_albumMap[b.Val()]))
				return false;
...
bool AlbumGenerator::_IsExcluded(const Album &ab, QString name)
{
	...
	for (auto &i : ab.items)
	{
		if ((i.IsAlbum()) && _albumMap[i.Val()].name == name)
			return _albumMap[i.Val()].ID.IsExcluded();
		else if ((i.IsImage()) && _imageMap[i.Val()].name == name)
			return _imageMap[i.Val()].ID.IsExcluded();
		else if ((i.IsVideo()) && _videoMap[i.Val()].name == name)
			return _videoMap[i.Val()].ID.IsExcluded();
	}
...
/*=============================================================
 * TASK   : move data from base album to first alias in pAlias
 *			(except thumbnail and text IDs) and make album an 
 *			alias of that
 * PARAMS :
 * EXPECTS:
 * GLOBALS:
 * RETURNS:
 * REMARKS: - if an alias album or one with alias(es) is deleted
 *				the images and folders inside it remain intact
 *				so no image or video usage count is decremented
 *				(as they were not incremented when the alias 
 *				album was created. Thumbnails and texts, on the 
 *				other hand can differ for base and alias albums
 *------------------------------------------------------------*/
void AlbumGenerator::_SwapAlbumWithAlias(Album& album, Album* pAlias)
{
	if (!pAlias)
		return;
	...
	int iSelf = pAlias->aliasesList.indexOf(pAlias->ID.Val());
	pAlias->aliasesList.remove(iSelf);   // and exlude pAlias from them
...
void AlbumGenerator::_RecursivelyReadSubAlbums(ID_t albumId)
{
	...
	Album &ab = _albumMap[albumId.Val()];
	...
	if (list.size())
		AddToModifiedList(albumId.Val(), true);

	ID_t id,
		 idt(ALBUM_ID_FLAG, albumId.Val());

	static int debugCnt = 0;
	for (auto &fi : list)		// elements in directory
	{
		++debugCnt;
		if( (id = _AddItemToAlbum(albumId.Val(), fi,_signalProgress)).Val() == 0)	// nothing added? (hidden file)
			continue;
		if (id.IsAlbum())		// sub album
		{
			Album& subA = _albumMap[id.Val()];
			subA.parentId = albumId.Val();		// signals this sub-album is in 'ab'
	...
	if (_processing)
	{
		...
		_AddAlbumThumbnail(ab, NO_ALBUM);	// only if there's no thumbnail already
	}
}
...
void AlbumGenerator::Init()
{
	...
	album.ID = { ALBUM_ID_FLAG, TOPMOST_ALBUM_ID };
	// album.ID.SetFlag(EXISTING_FLAG,false);	// do not check: virtual!
	_albumMap[TOPMOST_ALBUM_ID] = album;				// and not with _albumMap.Add()

	// add "latest" album if required

	album.ID = { ALBUM_ID_FLAG,  RECENT_ALBUM_ID };
...
//	im.ID.SetFlag(EXISTING_FLAG);
	im.name = "NoImage.jpg";
	im.ID = { IMAGE_ID_FLAG, NOIMAGE_ID };
	im.dsize = im.osize = QSize(800, 800);
	_imageMap[NOIMAGE_ID] = im;
...
bool  AlbumGenerator::SetChangesWritten()
{
	...
	_albumMap.SetChanged(INVALID_ALBUM_ID, false);
...
void AlbumGenerator::AddDirsRecursively(ID_t albumId)
{
	...
	AddToModifiedList(albumId.Val(), true);	// sets the changed flag and updates album only when not _processing
...
bool AlbumGenerator::Read(bool bMustReRead)
{
	...
	if (newDataExists)
	{
		...
		IDValList tmplst;
		for (auto &idv : _addedThumbnailIDsForImagesNotYetRead)
		{
			ID_t id(IMAGE_ID_FLAG, idv);
			if (Contains(id))
				++_imageMap[idv].thumbnailCount;
			else
				tmplst.push_back(idv);
		}
...
//				 & sDescTag  = (config.majorStructVersion == 1 && config.minorStructVersion < 4) ? OLD_DESCRIPTION_TAG : DESCRIPTION_TAG,
	...
	IDVal_t textID = 0;		// ID of last processed text
...
{
	QMap<IDVal_t,IDVal_t> pathIdVector;
	...
	QVector<IDVal_t> idsToDelete;
	IDPath_t pathID = pathMap.FirstId();
	do
	{
...
*--------------------------------------------------------------------------*/
static QStringList __imageMapStructLineToList(const QString &s)
{
	QStringList sl;
	bool oldType = false;
	...
	if (pos < 0)				// new file (maybe wasn't present before) or old format
	{
		pos = s.lastIndexOf('(');	// old format: start of parameters ('(' can appear in file name)
		if (pos >= 0)
		{							  
			oldType = true;
			pos0 = pos + 1;		// skip the '('
	...
	typ = FileTypeFromName(s.mid(0, pos));	// the part before the last '(' (old) or ('*') new name, based on file extension
	...
	if(pos < 0 && typ != ftUnknown)		// unprocessed line with known file type
		typ |= itemIsUnprocessedFlag;	// make it lowercase

	sl.push_back(s.mid(0, pos));	// index #0 - file name + extension, possibly with path
	sl.push_back(QChar(typ));		// index #1 - file type
...
* EXPECTS:	albumId - ID of album to add data to (parent)
...
*--------------------------------------------------------------------------*/
ID_t AlbumGenerator::AddItemToAlbum(IDVal_t albumId, QString path, bool isThumbnail, bool doSignalElapsedTime, bool doNotAddToAlbumItemList)
{
	if (!QDir::isAbsolutePath(path))
...
AlbumGenerator::AddedStatus AlbumGenerator::AddImageOrVideoFromString(QString fullFilePath, Album& album, bool onlyNew, int pos)
{
	...
	AddToModifiedList(album.ID.Val(), true);	// set it as changed always	 '_structFileChangeCount' is incremented in caller
	if (type == ftImage)
	{
		if (_imageMap.contains(id.Val()))
			img = _imageMap[id.Val()];
		else
			_imageMap.insert(id.Val(), img);
	...
	{
		if (_videoMap.contains(id.Val()))
			vid = _videoMap[id.Val()];
		else
			_videoMap.insert(id.Val(), vid);
...
*			in ID
* REMARKS:	- if the file was removed from disk in a pervious session and it is now 
* 				marked in the struct file as such, but added back again then 
*				it will be added as a new file
*			- line formats in reader.l() is described at '__imageMapStructLineToList'
...
ID_t AlbumGenerator::_ReadImageOrVideoFromStruct(FileReader &reader, int level, Album *album, bool thumbnail)
{
	...
	// if only the path name was given then n == 2
		//
	...
	ID_t id;
		// file type can be 'i' (= 'I' | 0x20) for images, 'v' for videos so remove 'itemIsUnprocessedFlag' flag here
	...
	if (n < 9 && n != 5 && n != 6)		   // maybe new image/video added and not yet processed
	{
		...
		{
			if (thumbnail && album)				// Its parent also changes
			{
				album->SetThumbnail(id);		// and increments 'thumbNailCount'
				if (album->parentId)
					AddToModifiedList(album->parentId, true ); // n == 2 always
			}
			AddToModifiedList(album->ID.Val(), true);			// set it as changed, always
			++_structFileChangeCount;
		}
		if (type == ftImage)
		{
			pItem = &_imageMap[id.Val()];
			if (added)
				ImageMap::lastUsedPathId = img.pathId;
		}
		else
		{
			pItem = &_videoMap[id.Val()];
			if (added)
				VideoMap::lastUsedPathId = vid.pathId;
		}
	}
	else  // n >= 9 or n == 5 or n == 6	processed video or image
	{
		...
		// calculated when asked for		img.aspect = img.height ? (double)img.width / (double)img.height : 1;
				...
				_imageMap[id.Val()] = img;
			}
			if(thumbnail)
				++_imageMap[id.Val()].thumbnailCount;
			else
				++_imageMap[id.Val()].usageCount;
			pItem = &_imageMap[id.Val()];
	...
	if (!reader.l().isEmpty() && reader.l()[level] == '[')
	{
		...
		if(nChanges != _structFileChangeCount && album)  // then image came from path in struct
			AddToModifiedList(album->ID.Val());
	}
...
*						2 - for virtual album: 'thisAlbum (A:ID)'  => name and ID 
*						3 - for physical album: 'name(A:ID)pathID' => name, ID, path
*						4 - for virtual album with base (possibly 0/empty path index):
*							'name(A:ID{baseID})pathID'  => name, ID, path, base ID: 
* REMARKS:	- "normal" regular function split() could not be used as names
*					may contain braces and commas
*			- result may contain fewer or more fields than required
*			- for root album sl is a single field, the ID which is 1
*			- when the album exists, but marked as 'C' ('changed') then its
*				parent (if it has any) - and only its parent -  must also marked changed
...
*			- throws 'BadStruct' on error
*--------------------------------------------------------------------------*/
IDVal_t AlbumGenerator::_ReadAlbumFromStruct(FileReader &reader, IDVal_t parent, int level)
{
	Album album = _albumMap[ TOPMOST_ALBUM_ID];	// temporary album to add to map, root album already added to map
	album.parentId = parent;				// won't get added again
	album.pathId = 0;
	...
	int n = sl.size();		// should be 1,2 or 3
							//	1: sl[0] = album name or full path name (not yet processed)
	...
	if (n == 1)		// either root album without a name and path, or full unprocessed path name 
	{
		...
		if (sl[0][0] != QChar('('))			// then path name for album
		{
			if (parent)
				AddToModifiedList(parent, true);		// parent must exist for each non-root album
	...
	else  // processed album not yet in albumMap: name, ID[, pathID[{base id}]
	{	  // parentid already set, n >= 2
		album.name = sl[0];
		id = sl[1].toLongLong();
		if(n > 2)	// path is present
		{
			...
			if (n == 4)
			{
				...
				else if( pBase->aliasesList.indexOf( id) < 0)
						pBase->aliasesList.push_back(id);	// add this album to base album's aliases list
			}
		}
		album.ID = { ALBUM_ID_FLAG, id };
		if(id != TOPMOST_PATH_ID && id != 2 && _albumMap.contains(id) && _albumMap[id].FullSourceName() != album.FullSourceName())
			throw BadStruct(reader.ReadCount(), QString("'%1'").arg(id) + FalconG::tr(" - duplicated album ID"));
		_albumMap[id] = album;	// add to map now so it can be used as parent for sub albums
	}
	...
	{								// in this case immediate parent must be changed too
		_albumMap[id].changed = true;
		AddToModifiedList(album);
		if (aParent != nullptr)
		{
			_albumMap[parent].changed = true;
			AddToModifiedList(parent, true);		// aParent must exist for each non-root album
		}
		++_structFileChangeCount;
	}
	...
	while (reader.Ok())
	{
		...
		{						   // so the previous record is finished
			if (!ids.IsEmpty())	   
			{
				if (ids.titleIDVal)
				...
				_AddAlbumThumbnail(album, ID_t(IMAGE_ID_FLAG,ids.thumbnailIDVal));
				ids.Clear();
			}
			...
			if (_albumMap.contains(id) && _albumMap[id].changed)	// if the album in _albumMap is not yet filled in, the changed flag still may be set
				AddToModifiedList(id, true);						// for it because of a sub-album
			_albumMap[id] = album;						// already added, but only update after all sub albums are processed
			// DEBUG
			//*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*
			//{
			...
			//		ofsdbg << sLevels << QString("   : items[%1] = (%2,%3)\n").arg(i).arg(_albumMap[id].items[i]).arg(_albumMap[id].items[i].Flags());
			//}
			//*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*
			// /DEBUG
		...
		{				// strings in lText must be empty
			if (reader.l()[level] == '[') // then title, description or thumbnail line
			{
				...
				if (changeCountBefore != _structFileChangeCount)
				{
					AddToModifiedList(album);
					if (parent)
						AddToModifiedList(parent, n == 2);	// aParent pointer might be invalidated
				}
			}
			...
			{							   // but image files are inside the album (one level down)
				...
				if (!iid.Val())
				{
					ShowWarning(FalconG::tr("Image/video not Found in line #%1").arg(reader.ReadCount()), frmMain);
...
void AlbumGenerator::_AddAlbumThumbnail(Album& album, ID_t id)
{
	...
	if (album.thumbnailId.Val() || album.ID.Val() == TOPMOST_ALBUM_ID)
		return;
	...
	AddToModifiedList(album);	 // now add the first item as a thumbnail
	...
	for (auto &a : album.items)
	{
		Album& alb = _albumMap[a.Val()];
		_AddAlbumThumbnail(alb, NO_ALBUM);
...
*		   - since Version 2.0.0 can't read older structure files any more
*				use separate conversion tool for that 
*--------------------------------------------------------------------------*/
	...
	try
	{
		...
		else
		{
			...
			if (rline[0] != '(' || rline[1] != 'A' && rline[1] != 'C')	// old type?
			{
				...
				if(sl1.size() > 2 && id.Val() != TOPMOST_ALBUM_ID)
			...
			_ReadAlbumFromStruct(reader, NO_ID, 0);	// parent ID is 0 for root album!
			_CleanupPathTable();
//#ifdef DEBUG
//			{
//				// DEBUG
...
//					s += QString(" for album %1 (id:%2)").arg(a.name).arg(a.ID);
//					map.insert(s, 1);
//				}
...
//					else
//						s += pathMap.Contains(a.pathId) ? pathMap[a.pathId] : QString("Invalid id:%1").arg(a.pathId);
//					s += QString(" for image %1 (id:%2)").arg(a.name).arg(a.ID);
//					map.insert(s, 1);
//				}
...
* EXPECTS:  bm   (def. BackupMode::wmReplace)		: should replace backup file with original
*			wm   (def.  WriteMode::wmOnlyIfChanged)	: should write even when didn't change? 
* GLOBALS:
...
int AlbumGenerator::WriteDirStruct(BackupMode bm, WriteMode wm)
{
	...
	pStructWiter = new AlbumStructWriter(*this, bm == BackupMode::bmKeepBackupFile);
	connect(pStructWiter, &AlbumStructWriter::SignalResultIsReady, this, &AlbumGenerator::_SlotForStructWriterFinished);
	connect(pStructWiter, &AlbumStructWriter::finished, pStructWiter, &QObject::deleteLater);
	_keepPreviousBackup = bm == BackupMode::bmKeepBackupFile;
...
/*============================================================================
  ...
  * REMARKS:
 *--------------------------------------------------------------------------*/
void AlbumGenerator::_WriteFacebookLink(QString linkName, IDVal_t ID)
{
...
{
	QString supdir = (album.ID.Val() == TOPMOST_ALBUM_ID ? "" : "../"),
	...
	QString sLang = languages["abbrev"]->size() > 1 ? ((*languages["language"])[_actLanguage]) : QString(),
			sLangU = (sLang.isEmpty() ? "" : "_" + sLang);

	if (album.ID.Val() == RECENT_ALBUM_ID)
	{
	...
	else if (album.items.size()) 
	{
		...
		s += QString(	" const imd='%1%2',"		// image dir
...
{
	QString updir = (album.ID.Val() == TOPMOST_ALBUM_ID) ? "" : "../";
...
//		+ (*languages["upOneLevel"])[_actLanguage] + "\"></a>\n";			  // UP link
	...
	if (album.ID.Val() > RECENT_ALBUM_ID && config.bGenerateLatestUploads && _latestImages.list.size())	// if there are no images in this category, do not add link
	{
		QString qs = "latest";
...
*--------------------------------------------------------------------------*/
int AlbumGenerator::_WriteGalleryContainer(Album & album, IDFlags typeFlag, int idIndex)
{
	...
	ID_t id = idIndex >= 0 ? idList[idIndex] : ID_t( ALBUM_ID_FLAG, RECENT_ALBUM_ID );
	...
	if (album.ID.Val() == TOPMOST_ALBUM_ID )
	{
	...
	{
		if (idIndex >= 0)
			thumb = ThumbnailID(_albumMap[id.Val()], _albumMap);
		else
		...
		if (thumb.Val())
		{
			if (_imageMap.contains(thumb.Val()))
				pImage = &_imageMap[thumb.Val()];
	...
	{
		pImage = &_imageMap[id.Val()];
		if (!pImage->Exists(CHECK))	
			pImage = &_imageMap[NOIMAGE_ID];
	}
	else		// video
	{
		pImage = &_imageMap[_videoMap[id.Val()].thumbnailId.Val()]; // video thumbnail is an image
		if (!pImage->Exists(CHECK))	
			pImage = &_imageMap[NOIMAGE_ID];
	}
	...
	{
		IDVal_t tid = isAlbum ? _albumMap[id.Val()].titleID : _imageMap[id.Val()].titleID,
				did = isAlbum ? _albumMap[id.Val()].descID :  _imageMap[id.Val()].descID;
	...
	if (isAlbum)
		qsLoc = "javascript:LoadAlbum('" +
					sAlbumDir + _albumMap[id.Val()].HtmlNameFromID(id.Val(), _actLanguage, _aliasRootId, false);
	else
...
int AlbumGenerator::_WriteVideoContainer(Album& album, int i)
{
	...
	sThumbnailDir = (QDir::isAbsolutePath(config.dsThumbDir.ToString()) ? "" : sOneDirUp) + config.dsThumbDir.ToString();
	Video* pVideo = &_videoMap[id.Val()];	// must exist
...
// was 		 << " <body onload = \"falconGLoad(" + QString(album.ID == TOPMOST_ALBUM_ID  && !config.bFixedLatestThumbnail && config.bGenerateLatestUploads ? "0":"0") + ")\" onbeforeunload=\"BeforeUnload()\"";
	...
	if (album.ID.Val() == TOPMOST_ALBUM_ID && config.bGenerateLatestUploads)	// root album and last uploaded?
	{
...
int AlbumGenerator::_CreatePage(Album &album, int language, QString uplink, int &processedCount)
{
	...
	QString s;
	if ( album.ID.Val() == TOPMOST_ALBUM_ID)		// top level: use name from config
		...
		if (pBaseAlbum->SubAlbumCount() > 0)
		{
			...
			for (int i = 0; _processing && i < pBaseAlbum->items.size(); ++i)
			{
				if((pBaseAlbum->items[i].IsAlbum()) )
					_CreatePage(_albumMap[pBaseAlbum->items[i].Val()], language, uplink, processedCount);
			}
		}
		--_aliasLevel;
		_aliasRootId = NO_ID;
...
int AlbumGenerator::_DoLatestJs()
{
	...
	for (int lang = 0; lang < languages.LanguageCount(); ++lang)
	{
		...
		for (auto &latest : _latestImages.list)
		{
			...
			//{
			//	pvid = &_videoMap[idt];
			//	ofjs << "{ id:" << (idt) << ",w:" << config.thumbWidth << ",h:" << config.thumbHeight;
			//	if (pvid->titleID)
...
int AlbumGenerator::_DoLatestHelper(QString baseName, int lang)
{
	...
	Album albumLatest;
	albumLatest.ID = { ALBUM_ID_FLAG, RECENT_ALBUM_ID };
 ...
 * REMARKS:
 *------------------------------------------------------------*/
bool AlbumGenerator::IsCircular(const Album* pAlbum, const Album* pInHere)
{
	if (!pAlbum || !pInHere)
		return false;
...
int AlbumGenerator::ProcessAndWrite()
{
	...
	emit SignalAlbumStructChanged(_structFileChangeCount || _albumMap.IsChanged());   // all album and image data
...
* REMARKS:
*--------------------------------------------------------------------------*/
void AlbumGenerator::_SlotForStructWriterFinished(QString sErrorMessage, QString sStructPath, QString sStructTmp)
{
 ...
 * PARAMS:	fromAlbum			 - remove from this album's items
 ...
 *------------------------------------------------------------*/
void AlbumGenerator::_RemoveItem(Album& fromAlbum, int ix, bool fromDisk)
{
	QString path; 
	ID_t idSub = fromAlbum.items[ix];
	...
	{				  // must be removed, unless this is an alias
		...
		Album& subAlbum = _albumMap[idSub.Val()];
		...
		else   // for base albums: remove items unless there are aliases for it
		{
			...
			if (subAlbum.aliasesList.isEmpty())
				_RemoveAllItemsFrom(idSub.Val(), fromDisk);
			else	   // it has aliases: so just move all of its data to the first alias
			{
				Album* pAlias = AlbumForIDVal(subAlbum.aliasesList[0]);
				_SwapAlbumWithAlias(subAlbum, pAlias); // except thumbnail ID, texts or parents
			}
		}
		// fromAlbum thumbnail
		if (subAlbum.thumbnailId.Val() && _imageMap.contains(subAlbum.thumbnailId.Val()))
		{
			Image* img = &_imageMap[subAlbum.thumbnailId.Val()];
			--img->thumbnailCount;
			if (!img->usageCount && !img->thumbnailCount)
			{
				_imageMap.remove(fromAlbum.thumbnailId.Val());
		...
		// If the fromAlbum has a physical folder on disk, first try to move it to trash
		// but if it doesn't work, then remove the folder recursively
		// In both cases all files inside the fromAlbum folder will be deleted!
		if (fromDisk && subAlbum.pathId != NO_ID && !QFile::moveToTrash(path))
			RemoveFolderRecursively(path);		// so those must also be deleted

		_albumMap.remove(idSub.Val());
		fromAlbum.DecrementAlbumCount();
	}
	else 
	{
		if (idSub.IsImage())    // remove from 'fromAlbum'
		{
			Image* img = &_imageMap[idSub.Val()];
			...
			{	// if only used as thumbnail and the file is removed from disk, can't keep it
				...
				_imageMap.remove(idSub.Val());
				if (fromDisk && (img->usageCount || img->thumbnailCount) && !QFile::moveToTrash(path))
					QFile::remove(path);
			}
			fromAlbum.DecrementImageCount();
		}
		else if (idSub.IsVideo())    // remove from this fromAlbum
		{
			Video* vid = &_videoMap[idSub.Val()];
			if (!--vid->usageCount)
			{
				path = vid->FullSourceName();
				_videoMap.remove(idSub.Val());
				if (fromDisk && (vid->usageCount) && !QFile::moveToTrash(path))
					QFile::remove(path);
			}
			fromAlbum.DecrementVideoCount();
		}
	}
	// remove invalid item
	fromAlbum.items.remove(ix);
}
 ...
 *			- tries to move files into trashcan^recycle bin first
 *------------------------------------------------------------*/
void AlbumGenerator::_RemoveAllItemsFrom(IDVal_t albumID , bool fromDisk)
{
	...
	album.items.clear();
	albumgen.AddToModifiedList(albumID, true);
}
/*=============================================================
 * TASK:   recursive source album, and image/video deletion
 *			from data base and conditionally from disk
 ...
 *			- tries to move files into trashcan/recycle bin first
 *------------------------------------------------------------*/
void AlbumGenerator::_RemoveItems(IDVal_t albumID, bool iconsForThisAlbum, IntList ilx, bool fromDisk)
{
	...
	album.RecalcItemCounts();			// counts would reflect the number of items
	albumgen.AddToModifiedList(albumID);
}
...
bool AlbumGenerator::AddFolder(IDVal_t albumId, QString folderName)
{
	...
	if (added)
	{
		...
		if (idth.Val())
			...
			folderName = Images()[idth.Val()].FullSourceName();
...
void AlbumGenerator::AddToModifiedList(IDVal_t albumId, bool itemNotProcessedYet)		// albumId must be valid
{
	if (_processing && !itemNotProcessedYet)
		return;

	_albumMap.SetChanged(albumId);
