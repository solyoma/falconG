...
QRect SlideWidget::MatteRect() const
{
    if (_keepAspectRatio)
    {
...
void SlideWidget::paintEvent(QPaintEvent*)
{
    ...
    outerPath.addRoundedRect(rect(), radius, radius);

    // border
    p.fillPath(outerPath, _borderColor);

    // matte border
    radius = EffectiveRadius(true);
    QPainterPath MattePath;
    MattePath.addRoundedRect(MatteRect(), radius, radius);
...
void SlideWidget::SlotSetBorder(int width, QColor color, qreal radius, RadiusUnit unit)
{
    _borderWidth = width;
    _borderColor = color;
